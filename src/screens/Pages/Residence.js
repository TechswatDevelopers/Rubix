...
class Residence extends React.Component {
  ...
  fetchImages(resID) {
    ...
    await fetch('https://rubixdocumentsdev.cjstudents.co.za:86/feed/post/' + resID)
      ...
      for(let i = 0; i <= data.post.length - 1; ++i){
       ...
       if(data.post[i].FileType != "ResManager"){ 
         this.state.gallery.push(
          {
            original: 'https://rubiximagesdev.cjstudents.co.za:449/' + data.post[i].filename,
            thumbnail: 'https://rubiximagesdev.cjstudents.co.za:449/' + data.post[i].filename
          }
        )
        }
        else (
          this.setState({
            resManagerPic: 'https://rubiximagesdev.cjstudents.co.za:449/' +  data.post[i].filename
          })
        )
      }
      /* this.setState({
