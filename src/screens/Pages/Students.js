...
class Students extends React.Component {
    constructor(props) {
        ...
        this.state = {
          ...
          studentLeaseAmmend: [],
          listIndex: 0,
          occups: ['New', 'Existing', 'Unverified'],
          occupancy: '',
        }
      }
  ...
  componentDidMount() {
    ...
    .then(data => {
        ...
        let temp = []
        ///Check user ID to load reslist
        if(localStorage.getItem('adminID') == '4' || localStorage.getItem('adminID') == 4 || localStorage.getItem('adminID') == '5' || localStorage.getItem('adminID') == 5)
        {
          //console.log("data is ", data.data)
          let thisData = data.data
          thisData.forEach(res => {
            //console.log(res.RubixResidenceID)
            if(res.RubixResidenceID == '1' || res.RubixResidenceID == '2' || res.RubixResidenceID == 1 || res.RubixResidenceID == 2 || res.RubixResidenceID == null){
...
postData().then(()=>{
  ...
  exportToCSV(resID){
    ...
    'ResidenceRoomNameFull': 'ResidenceRoomNameFull',
     'Capacity': 'Capacity',
      'FileType': 'FileType',
